<template>
	<div class="login-register">
			...
			<div class="big-box" :class="{ active: isLogin }">
				<div class="big-contain" v-if="isLogin">
					...
					<div class="bform" v-if="isForgot">
						...
						<transition name="fade-transform"><span class="errTips" v-if="Login_Error_Tip">* 登录信息有误 *</span></transition>
					</div>
					<!-- 忘记密码的表单 -->
					<div class="bform" v-else>
						...
						<transition name="fade-transform"><span class="errTips" v-if="Login_Error_Tip">* 重置信息有误 *</span></transition>
					</div>
					<div v-if="isForgot"><el-button type="success" class="vbutton" @click="login">登录</el-button></div>
					<!-- 忘记密码的按钮部分 -->
					<div v-else>
						<el-button type="success" class="vbutton" @click="reset_Password">重置</el-button>
						<el-button type="success" class="vbutton" @click="send_Forgot_Email">发射验证码</el-button>
					</div>
				</div>
				<div class="big-contain" v-else>
					...
					<div>
						<transition name="slide-fade"><el-button type="primary" class="vbutton" v-show="is_Show_Register_Btn" @click="register">注册</el-button></transition>
						<el-button type="success" class="vbutton" v-if="!countDown" @click="send_Email">发射验证码</el-button>
						<el-button type="info" disabled="disabled" v-else="countDown">已发射 {{ countDown }}s</el-button>
					</div>
				</div>
			</div>
			<div class="small-box" :class="{ active: isLogin }">
				<div class="small-contain" v-if="isLogin">
					<div class="stitle"><img src="~/assets/logoFont_v2.1.png" style="width:7rem" /></div>
		...
		<strong class="footer">© 2021 闽ICP备20007479号-1</strong>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		async register() {
			...
			if (username == '' && email == '' && password == '' && verify == '') return this.alert_No_Null();
...
* {
	font-family: 'PingFangSC-Regular', 'PingFang SC', core_sans_n45_regular, 'Avenir Next', 'Helvetica Neue', Helvetica, Arial, 'Source Han Sans SC', 'Hiragino Sans GB',
